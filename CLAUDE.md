...
- `docs/current_status.md` — where the project actually is today: B1 RSSM works (h=10 open-loop FG-MSE ≈ 0.005); mechanics model went through v2/v3/v4/diag debugging iterations (z-channel phase transition diagnosed and fixed via `nuisance_dim = 4` + `nuisance_min_std = 1.0`); v4 trained stably to K1 ≈ 0.008 but the `(q, q̇)` factorization was nominal — `q̇` drifted because the soft smoothness loss couldn't beat reconstruction pressure. Phase-3 architecture now derives `q̇ = d/dt(q)` by construction via central-difference of `q` inside the encoder; smoothness loss deleted. Next run is the Phase-3 training → `eval-mechanics` with K1/K2/H3 on the recollected-with-physics dataset.
...
- **Mechanics (`MechanicsWorldModel`, `src/models/mechanics/`)** composes the shared `ConvEncoder` trunk with a `FactoredEncoder` that emits Gaussian posterior params for `q` (`q_dim=2` for cartpole) and nuisance `z` (`nuisance_dim=4` default on cartpole). **`q̇` is derived, not emitted** — `derive_qdot_from_q(q_mean, q_std, dt)` in `encoder.py` computes `q̇_t = (q_{t+1} − q_{t-1}) / (2·dt)` (central diff interior, forward/backward at the boundaries) and the Gaussian pushforward `q̇_std = sqrt(q_std_{t+1}² + q_std_{t−1}²) / (2·dt)`. Encoder requires `T ≥ 2`; `initial_posterior` / `posterior_step` accept `[B, T≥2, C, H, W]` and return the posterior at the last frame. `lagrangian.py` hosts Cholesky-PD `MassMatrix` (ridge `eps=1e-2`), learned `Potential`, PSD `Dissipation`, learned `Actuation`; all four MLPs tanh-bound their inputs (q scale 5, qdot scale 20) to prevent cold-start dynamics blow-up (F2 defensive fix). `forward_acceleration` implements the mechanical-form Euler-Lagrange step using autograd.grad (per-dim loop avoids the MPS bugs in `torch.autograd.functional.jvp`). `integrator.py` is a semi-implicit symplectic Euler step. `transition.py` composes the integrator on `(q, q̇)` with an AR(1) nuisance channel; its `prior_min_std_qdot` defaults to `prior_min_std / dt` so the integrator's prior q̇-std matches the encoder's pushforward scale at init. The decoder input is `concat(q, q̇, z)` of size `2 * q_dim + nuisance_dim`. Reward head is also opt-in. Phase-0 imagination-rollout contract is preserved exactly so `eval-control` / CEM plug in unchanged. Critical MPS detail: **do not reintroduce `torch.linalg.solve` inside the imagination loop** — it corrupts kernel dispatch under autograd.grad; a regression test in `tests/test_mechanics_primitives.py` guards this.

**Training and loss (`src/train/`, `src/models/losses.py`, `src/models/mechanics/losses.py`)** — offline only. The RSSM composite loss combines KL (balanced + free-nats), posterior reconstruction, **foreground-masked** reconstruction (background-subtracted saliency mask), dynamic-mask reconstruction, transition reconstruction (decoded prior features at `t` targeted against `obs_{t+1}`), foreground-masked transition reconstruction, **imagination reconstruction** (decoded prior features over `K_ctx + K_imag` horizon), optional reward prediction, and BYOL-style latent consistency. The mechanics loss uses a three-branch balanced KL with asymmetric free-nats (tight on `q`/`q̇`, loose on `z`), posterior / foreground / imagination reconstruction, and optional reward — **no smoothness prior** (the kinematic constraint `q̇ = d/dt(q)` is now structural, not a soft loss term). Each term has its own CLI weight flag. The mechanics trainer also includes a NaN/Inf skip guard (F1 defensive fix) so a pathological batch cannot poison AdamW's running moments. Both trainers use AdamW with linear warmup + cosine LR, write per-epoch and `latest.pt` checkpoints plus JSONL `history.jsonl`, support `--auto-resume`, and log a `val_open_loop_fg_mse_h*` probe every N steps so the K1 metric is observable live. The mechanics history also carries z-channel diagnostics (`z_post_mean_rms`, `kl_z_t0_mean`, `kl_z_transition_mean`, ...) surfaced during the v3 debugging. **Keep JSONL history machine-readable** — terminal formatting (Rich colors, timestamps) must not change the schema.
